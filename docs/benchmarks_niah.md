...
(`IdTokenBiMap(keep_bits=None)`) with the library's default `keep_bits=30` encoding; raw `hex`,
...
- Default encodings: `utf_token` (full-input) and `utf_token_keep_30` (the library default).
